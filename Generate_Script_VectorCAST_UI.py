...
class VectorCastGeneratorUI:
    def __init__(self, root):
        ...
        self.header_folder_var = tk.StringVar()
        self.unit_name_var = tk.StringVar()
        self.env_name_var = tk.StringVar(value="TEST_ENV")
    ...
    def _create_widgets(self):
        ...
        # Row 0: C File
        ttk.Label(main_frame, text="C Source File:").grid(row=0, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.c_file_var).grid(row=0, column=1, sticky=tk.EW, padx=5)
        ttk.Button(main_frame, text="Browse...", command=self._browse_c_file).grid(row=0, column=2)
        
        # Row 1: Header Folder
        ttk.Label(main_frame, text="Header Folder:").grid(row=1, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.header_folder_var).grid(row=1, column=1, sticky=tk.EW, padx=5)
        ttk.Button(main_frame, text="Browse...", command=self._browse_header_folder).grid(row=1, column=2)
        
        # Row 2: Unit Name
        ttk.Label(main_frame, text="Unit Name (UUT):").grid(row=2, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.unit_name_var).grid(row=2, column=1, sticky=tk.EW, padx=5)
        
        # Row 3: Environment Name
        ttk.Label(main_frame, text="Environment Name:").grid(row=3, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.env_name_var).grid(row=3, column=1, sticky=tk.EW, padx=5)
        
        # Row 4: Output File
        ttk.Label(main_frame, text="Output Filename:").grid(row=4, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.output_file_var).grid(row=4, column=1, sticky=tk.EW, padx=5)
        
        # Row 5: Options
        ttk.Checkbutton(main_frame, text="Verbose Logging (Debug)", variable=self.verbose_var).grid(row=5, column=0, columnspan=2, sticky=tk.W, pady=10)
        
        # Row 6: Run Button
        self.run_btn = ttk.Button(main_frame, text="GENERATE SCRIPT", command=self._start_generation_thread)
    ...
    def _browse_header_folder(self):
        folder = filedialog.askdirectory()
        if folder:
            self.header_folder_var.set(folder)
    ...
    def _run_generation(self):
        ...
        if not c_file or not header_folder or not unit_name:
            logger.error("Missing required fields: C File, Header Folder, or Unit Name.")
        ...
        try:
            generator = VectorCastGenerator(c_file, header_folder, unit_name, env_name)
